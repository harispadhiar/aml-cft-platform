...
func (dl *downloader) RefreshAll(ctx context.Context) (Stats, error) {
	...
	// PK NACTA Records (Mocked)
	if slices.Contains(requestedLists, search.SourcePKNACTA) {
		listsLoaded = append(listsLoaded, search.SourcePKNACTA)
		...
		g.Go(func() error {
			...
			// Mock data
			list := preparedList{
				ListName: search.SourcePKNACTA,
				Hash:     "mock-nacta-hash",
			}
			list.Entities = append(list.Entities, search.Entity[search.Value]{
				Name:   "Mocked NACTA Terrorist",
				Type:   search.EntityPerson,
				Source: search.SourcePKNACTA,
			})
			preparedLists <- list
			return nil
		})
	...
	// PK SBP Records (Mocked)
	if slices.Contains(requestedLists, search.SourcePKSBP) {
		listsLoaded = append(listsLoaded, search.SourcePKSBP)
		...
		g.Go(func() error {
			...
			// Mock data
			list := preparedList{
				ListName: search.SourcePKSBP,
				Hash:     "mock-sbp-hash",
			}
			list.Entities = append(list.Entities, search.Entity[search.Value]{
				Name:   "Mocked SBP Proscribed",
				Type:   search.EntityPerson,
				Source: search.SourcePKSBP,
			})
			preparedLists <- list
			return nil
		})
